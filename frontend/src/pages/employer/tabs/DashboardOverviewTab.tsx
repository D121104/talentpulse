...
export function DashboardOverviewTab({
  ...
  const todayPosted = stats?.todayJobsPostedCount ?? 0;
  const maxDailyJobs = stats?.maxDailyJobs ?? 5;
  const remainingQuota = Math.max(0, maxDailyJobs - todayPosted);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="font-bold text-slate-800 dark:text-slate-200">
              {todayPosted}/{maxDailyJobs} {t('employer.sidebar.freeTierDailyJobs')}
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <span>{t('employer.jobsTab.quotaLimit')}:</span>
            <span className="font-bold text-emerald-600 dark:text-emerald-400">
              {remainingQuota} {t('employer.sidebar.freeTierDailyJobs')}
            </span>
          </div>
        </motion.div>
      </div>
          ...
          <div className="rounded-3xl border border-slate-200/90 bg-gradient-to-br from-slate-900 to-slate-950 p-5 text-white shadow-lg shadow-slate-950/20">
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 text-xs flex justify-between text-slate-300">
              ...
              <span className="font-bold text-emerald-400">
                {todayPosted}/5 {t('employer.sidebar.freeTierDailyJobs')}
              </span>
            </div>
          </div>
        </div>
