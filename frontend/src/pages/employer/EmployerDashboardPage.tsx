...
export default function EmployerDashboardPage() {
  ...
  const handleOpenCreateJob = () => {
    ...
    if (
      statsData?.stats &&
      statsData.stats.maxDailyJobs < 999 &&
      statsData.stats.activeJobs >= (statsData.stats.maxActiveJobs || 6)
    ) {
      info(
        `Tài khoản HR miễn phí chỉ được tối đa ${statsData.stats.maxActiveJobs || 6} tin tuyển dụng đang hoạt động cùng lúc (${statsData.stats.activeJobs}/${statsData.stats.maxActiveJobs || 6}). Hãy nâng cấp HR Premium để đăng tin không giới hạn!`,
      );
      handleNavigateTab('premium');
      return;
    }
