...
export const onlineCvApi = {
  ...
  exportPdf: (
    ...
    apiRequest<{ _id: string; pdfUrl: string; message: string }>(
      `/online-cvs/${id}/export`,
      {
        method: "POST",
  ...
  toggleSearchable: (
    ...
    apiRequest<{ _id: string; isSearchable: boolean; message: string }>(
      `/online-cvs/${id}/searchable`,
      {
        method: "PATCH",
        body: { isSearchable },
        accessToken,
