...
export const employerApi = {
  ...
  getDashboardStats: (accessToken: string) =>
    apiRequest<HrDashboardStats>("/companies/hr/dashboard-stats", {
      accessToken,
    }),
  ...
  updateCompany: (
    ...
    apiRequest<CompanyInfo>(`/companies/${id}`, {
      method: "PATCH",
      body: data,
      accessToken,
    }),
  ...
  getAllCompanies: (query = "", accessToken?: string) =>
    apiRequest<{ meta: any; result: CompanyInfo[] }>(`/companies?${query}`, {
      accessToken,
    }),
  ...
  getPendingHrs: (companyId: string, accessToken: string) =>
    apiRequest<PendingHrRequest[]>(`/companies/${companyId}/pending-hrs`, {
      accessToken,
    }),

  approveHr: (companyId: string, userId: string, accessToken: string) =>
    apiRequest<{ message: string }>(
      `/companies/${companyId}/approve-hr/${userId}`,
      { method: "POST", accessToken },
    ),

  rejectHr: (companyId: string, userId: string, accessToken: string) =>
    apiRequest<{ message: string }>(
      `/companies/${companyId}/reject-hr/${userId}`,
      { method: "POST", accessToken },
    ),
  ...
  leaveCompany: (accessToken: string) =>
    apiRequest<{ message: string }>("/users/leave-company", {
      method: "POST",
      accessToken,
    }),
  ...
  getHrJobs: (
    ...
    return apiRequest<{ meta: any; result: HrJobItem[] }>(
      `/jobs/by-hr/all?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  getSkills: (
    ...
    return apiRequest<{ meta: any; result: SkillItem[] }>(
      `/skills?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  getApplications: (
    ...
    return apiRequest<{ meta: any; result: ApplicationItem[] }>(
  ...
  getApplicationsByJob: (
    ...
    if (params.status) qs.append("status", params.status);
    return apiRequest<{ meta: any; result: ApplicationItem[] }>(
      `/applications/by-job/${jobId}?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  searchCandidatesByCV: (
    ...
    return apiRequest<{ total: number; result: any[] }>(
      `/applications/by-job/${jobId}/search-cv?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  getNotifications: (page = 1, limit = 15, accessToken: string) =>
    apiRequest<{ meta: any; result: NotificationItem[] }>(
  ...
  searchCandidatesGlobal: (
    ...
    return apiRequest<{ meta: any; result: CandidatePublicItem[] }>(
      `/hr/candidates?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  unlockCandidate: (
    ...
    apiRequest<UnlockedCandidateInfo>(
      `/hr/candidates/${candidateUserId}/unlock`,
      {
        method: "POST",
        body: data,
        accessToken,
  ...
  getCandidateUnlockHistory: (
    ...
    if (params.current) qs.append("current", String(params.current));
    if (params.pageSize) qs.append("pageSize", String(params.pageSize));
    return apiRequest<{ meta: any; result: any[] }>(
      `/hr/candidates/my-unlocks?${qs.toString()}`,
      { accessToken },
    );
  },

  // 10. Candidate Profile Views History & Job Seeking Settings
  getCandidateEmployerViews: (
    params: { current?: number; pageSize?: number } = {},
    accessToken: string,
  ) => {
    const qs = new URLSearchParams();
    if (params.current) qs.append("current", String(params.current));
    if (params.pageSize) qs.append("pageSize", String(params.pageSize));
    return apiRequest<CandidateEmployerViewsResponse>(
      `/candidate/employer-views?${qs.toString()}`,
      { accessToken },
    );
  },
  ...
  updateCandidateJobSettings: (
    ...
    return apiRequest<{ message: string; settings: any }>(
